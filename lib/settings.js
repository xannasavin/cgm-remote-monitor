...
function init () {
  ...
  var settings = {
    ...
    , ai_llm_api_url: 'https://api.openai.com/v1/chat/completions' // API URL for the LLM
    , ai_llm_model: 'gpt-4o' // Default LLM model
    , ai_llm_temperature: 0.0 // Default temperature for LLM
    , ai_llm_max_tokens: 2000 // Default max tokens for LLM response
    ...
    , ai_llm_exchangerate_api_polling_interval: 7 // Days between API calls
    , ai_llm_exchangerate_api_poling_intervall: 7 // backward-compat alias (typo)
    , ai_llm_monthly_usd_limit: 20 // Default monthly USD limit
    , ai_llm_default_display: 'Show all results'
  };
  ...
  var valueMappers = {
    ...
    , ai_llm_temperature: mapNumber // Added mapper for ai_llm_temperature
    , ai_llm_max_tokens: mapNumber  // Added mapper for ai_llm_max_tokens
    ...
    , ai_llm_exchangerate_api_polling_interval: mapNumber
    , ai_llm_exchangerate_api_poling_intervall: mapNumber
    , ai_llm_monthly_usd_limit: mapNumber
  };
  ...
  // Set ai_llm_key_is_set based on the presence of ai_llm_key
  // This should be done after environment variables have populated settings.ai_llm_key
  // and before settings are filtered for the client.
  // A typical place this happens is after all calls to eachSettingAsEnv(env_accessor)
  // or just before returning the settings object if env vars are processed by then.
  // For this modification, we'll place it right before returning settings,
  // assuming settings.ai_llm_key has its value from the environment at this point.
  if (settings.ai_llm_key && typeof settings.ai_llm_key === 'string' && settings.ai_llm_key.trim() !== '') {
    settings.ai_llm_key_is_set = true;
  }
  // No 'else' needed as it defaults to false.

  if(settings.ai_llm_debug === true) {
    console.log('AI Eval DEBUG Settings before return:', settings);
  }
