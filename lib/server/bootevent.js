...
function boot (env, language) {
  ...
  function ensureIndexes (ctx, next) {
    ...
    // AI usage indexes (Finding #18)
    try {
      var aiUsageCol = ctx.store.collection('ai_usage_stats');
      if (aiUsageCol && aiUsageCol.createIndex) {
        aiUsageCol.createIndex({ createdAt: -1 }, { background: true }, function (err) {
          if (err) console.error('Unable to create index for ai_usage_stats.createdAt:', err);
        });
      }
      var exchangeRatesCol = ctx.store.collection('exchange_rates');
      if (exchangeRatesCol && exchangeRatesCol.createIndex) {
        exchangeRatesCol.createIndex(
          { base_currency: 1, target_currency: 1 },
          { unique: true, background: true },
          function (err) {
            if (err) console.error('Unable to create index for exchange_rates:', err);
          }
        );
      }
    } catch (e) {
      console.warn('AI plugin indexes not created:', e.message);
    }
