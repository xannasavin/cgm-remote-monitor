...
function resolvePath(filePath) {
  ...
  // eslint-disable-next-line security/detect-non-literal-fs-filename -- callers pass hardcoded asset paths; resolvePath is a trusted internal helper
  if (fs.existsSync(filePath)) return filePath;
  let p = path.join(__dirname, filePath);
  // eslint-disable-next-line security/detect-non-literal-fs-filename -- same as above: trusted internal asset resolution
...
function create (env, ctx) {
  ...
  if (!insecureUseHttp) {
    ...
    if (secureHstsHeader) { // Add HSTS (HTTP Strict Transport Security) header
      ...
      if (enableCSP) {
        ...
        // Default body-parser limit is 100kb, which is too tight for the AI
        // eval endpoint (7+ days of CGM data + prompts + schema easily blows
        // past 100kb). Bump to 10mb so longer reports go through without a
        // 413. Individual routes (entries, treatments, activity) still set
        // their own 50mb limits where payload sizes can get larger.
        app.use(bodyParser.json({ type: ['json', 'application/csp-report'], limit: '10mb' }));
  ...
  // eslint-disable-next-line security/detect-non-literal-fs-filename -- hardcoded asset path passed through resolvePath helper
  ...
  // eslint-disable-next-line security/detect-non-literal-fs-filename -- hardcoded swagger asset path relative to __dirname
