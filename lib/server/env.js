...
function config () {
  ...
  env.ai_llm_temperature = parseFloat(readENV('AI_LLM_TEMPERATURE', 0));
  env.ai_llm_max_tokens = parseInt(readENV('AI_LLM_MAX_TOKENS', 2000), 10);
  ...
  env.ai_llm_monthly_usd_limit = parseFloat(readENV('AI_LLM_MONTHLY_USD_LIMIT', 20));
  env.ai_llm_timeout = parseInt(readENV('AI_LLM_TIMEOUT', 120), 10); // Timeout in seconds for LLM API calls
