...
function config () {
  ...
  Object.keys(process.env).forEach((key, _index) => {
  ...
  // Add AI settings
  env.ai_llm_key = readENV('AI_LLM_KEY');
  env.ai_llm_api_url = readENV('AI_LLM_API_URL');
  env.ai_llm_model = readENV('AI_LLM_MODEL');
  var rawTemp = parseFloat(readENV('AI_LLM_TEMPERATURE', 0));
  env.ai_llm_temperature = isNaN(rawTemp) ? 0 : rawTemp;
  var rawMaxTokens = parseInt(readENV('AI_LLM_MAX_TOKENS', 2000), 10);
  env.ai_llm_max_tokens = isNaN(rawMaxTokens) ? 2000 : rawMaxTokens;
  env.ai_llm_debug = readENVTruthy('AI_LLM_DEBUG', false);
  env.ai_llm_default_display = readENV('AI_LLM_DEFAULT_DISPLAY', 'Show all results');
  env.ai_llm_exchangerate_api_key = readENV('AI_LLM_EXCHANGERATE_API_KEY');
  env.ai_llm_exchangerate_api_currency = readENV('AI_LLM_EXCHANGERATE_API_CURRENCY');
  var rawPolling = parseInt(readENV('AI_LLM_EXCHANGERATE_API_POLING_INTERVALL', 7), 10);
  env.ai_llm_exchangerate_api_polling_interval = isNaN(rawPolling) ? 7 : rawPolling;
  var rawApiLimit = parseInt(readENV('AI_LLM_EXCHANGERATE_API_LIMIT', 100), 10);
  env.ai_llm_exchangerate_api_limit = isNaN(rawApiLimit) ? 100 : rawApiLimit;
  var rawLimit = parseFloat(readENV('AI_LLM_MONTHLY_USD_LIMIT', 20));
  env.ai_llm_monthly_usd_limit = isNaN(rawLimit) ? 20 : rawLimit;
  var rawTimeout = parseInt(readENV('AI_LLM_TIMEOUT', 120), 10);
  env.ai_llm_timeout = isNaN(rawTimeout) ? 120 : rawTimeout;
  env.ai_llm_provider = readENV('AI_LLM_PROVIDER'); // Explicit provider override: 'openai', 'anthropic'

  // Plugin isolation: set flag for client-side visibility (Step 3.4)
  env.settings.ai_llm_key_is_set = !!env.ai_llm_key;

  if (env.ai_llm_key) {
    console.log('AI plugin: enabled');
  } else {
    console.log('AI plugin: disabled (AI_LLM_KEY not set)');
  }
...
function setSSL () {
  ...
  if (env.SSL_KEY && env.SSL_CERT) {
    env.ssl = {
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- path from trusted operator env var SSL_KEY
      key: fs.readFileSync(env.SSL_KEY)
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- path from trusted operator env var SSL_CERT
      , cert: fs.readFileSync(env.SSL_CERT)
    };
    if (env.SSL_CA) {
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- path from trusted operator env var SSL_CA
