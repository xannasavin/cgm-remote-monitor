...
function computeDayStats (sgvRecords, treatments, options) {
  ...
  // Time blocks (hourly)
  var time_blocks = computeTimeBlocks(sorted);
...
function computeMAGE (sorted, sd, validHours) {
  ...
  var turningPoints = [];
  turningPoints.push({ mills: sorted[0].mills, sgv: sorted[0].sgv });
  ...
  for (var i = 1; i < sorted.length; i++) {
    ...
    if (gap > GAP_THRESHOLD_MS) {
      // Gap: reset direction tracking without adding boundary points.
      // Adding gap boundaries as turning points creates artificial excursions.
      prevDirection = 0;
      continue;
    }
  ...
  // Add the last point
  turningPoints.push({ mills: sorted[sorted.length - 1].mills, sgv: sorted[sorted.length - 1].sgv });
 ...
 */
function computeTimeBlocks (sorted) {
  var blocks = {};

  for (var i = 0; i < sorted.length; i++) {
    var hour = new Date(sorted[i].mills).getUTCHours();
    if (!blocks[hour]) {
...
function computePeriodStats (dayStatsArray) {
  ...
  var periodSd = allAverages.length > 1 ? ss.standard_deviation(allAverages) : 0;
  var periodCv = periodAvg !== 0 ? (periodSd / periodAvg) * 100 : null;
