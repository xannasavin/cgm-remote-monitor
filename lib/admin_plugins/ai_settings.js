    ...
    actions: [
      {
        ...
        init: function(client) { // client is Nightscout.client
          ...
          // Add the interim Prompts
          const systemInterimPromptId = `ai_system_interim_prompt_${plugin.name}`;
          const userPromptInterimTemplateId = `ai_user_prompt_interim_template_${plugin.name}`;
          ...
          $formContainer.append(
              $('<div>').css('margin-bottom', '15px').append(
                  $('<label>').attr('for', systemInterimPromptId).css({display: 'block', marginBottom: '5px'}).text(client.translate('System Interim Prompt:')), // Reverted to use client.translate
                  $('<textarea>').attr('id', systemInterimPromptId).attr('rows', 5).css({width: '90%', fontFamily: 'monospace'}).attr('placeholder', client.translate('Used to analyze and summrize the single days before sending via the final prompt.'))
              )
          );
          $formContainer.append(
              $('<div>').css('margin-bottom', '15px').append(
                  $('<label>').attr('for', userPromptInterimTemplateId).css({display: 'block', marginBottom: '5px'}).text(client.translate('User Interim Prompt:')), // Reverted to use client.translate
                  $('<textarea>').attr('id', userPromptInterimTemplateId).attr('rows', 5).css({width: '90%', fontFamily: 'monospace'}).attr('placeholder', client.translate('Used to analyze and summrize the single days before sending via the final prompt.'))
              )
          );
          ...
          const $userInterimPromptEl = $formContainer.find('#' + userPromptInterimTemplateId);
          ...
          $.ajax({ // Still using $ which is client.jquery
            ...
            success: function(data) {
              ...
              if ($userInterimPromptEl.length) {
                $userInterimPromptEl.val(data.user_prompt_interim_template || '');
              } else {
                console.error("Admin AI Settings: #" + userPromptInterimTemplateId + " not found after injection.");
              }
        ...
        code: function(client, buttonElement) {
          ...
          const systemInterimPromptId = `ai_system_interim_prompt_${plugin.name}`;
          const userPromptInterimTemplateId = `ai_user_interim_prompt_template_${plugin.name}`;
          const systemInterimPrompt = $('#' + systemInterimPromptId).val();
          const userPromptInterimTemplate = $('#' + userPromptInterimTemplateId).val();
          ...
          $.ajax({
            ...
            data: JSON.stringify({
              ...
              user_interim_prompt_template: userPromptInterimTemplate
            }),
