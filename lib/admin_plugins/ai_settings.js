    ...
    actions: [
      {
        ...
        description: '<p>There are four prompts to be set:</p>' +
            '<ol><li>The interim System- and User-Prompt</li>' +
            '<li>The final System- and User-Prompt</li></ol>' +
            '<p>The interim prompt is used to send the daily data to AI which returns a summarized analyization</p>' +
            '<p>The final prompts then send this summarized dataset to AI for an overall analyzation of the chosen timeframe</p>',
        ...
        init: function(client) { // client is Nightscout.client
          ...
          $formContainer.append(
              $('<div>').css('margin-bottom', '15px').append(
                  $('<label>').attr('for', userInterimPromptTemplateId).css({display: 'block', marginBottom: '5px'}).text(client.translate('User Interim Prompt (use {{CGMDATA}} for the glucose & treatment data and {{PROFILE}} for the insulin profile data):')),
          ...
          $formContainer.append(
              $('<div>').css('margin-bottom', '15px').append(
                  $('<label>').attr('for', userPromptTemplateId).css({display: 'block', marginBottom: '5px'}).text(client.translate('User Prompt (use {{INTERIMAIDATA}} for the AI\'s summary and {{PROFILE}} for the insulin profile data):')),
          ...
          $formContainer.append(
              $('<p>').append($('<em>').text(client.translate('Note: The Tokens {{CGMDATA}}, {{PROFILE}} and {{INTERIMAIDATA}} will be replaced with the actual JSON data from the selected report period when the AI evaluation is generated.')))
          );
        ...
        code: function(client, buttonElement) {
          ...
          const systemPromptId = `ai_system_prompt_${plugin.name}`;
          const userPromptTemplateId = `ai_user_prompt_template_${plugin.name}`;
          const systemInterimPrompt = String($('#' + systemInterimPromptId).val() || '');
          const userInterimPromptTemplate = String($('#' + userInterimPromptTemplateId).val() || '');
