    ...
    actions: [
      {
        ...
        init: function(client) { // client is Nightscout.client
          ...
          $formContainer.append(
              $('<div>').css('margin-bottom', '15px').append(
                  $('<label>').attr('for', userInterimPromptTemplateId).css({display: 'block', marginBottom: '5px'}).text(client.translate('User Interim Prompt (use {{CGMDATA}} the glucose & treatment data and {{PROFILE}} for the insulin profile data):')),
