    ...
    actions: [
      {
        ...
        description: '<p>Configure the two prompts sent to the LLM for CGM data analysis:</p>' +
            '<ol><li>The <strong>System Prompt</strong> defines the LLM\'s role, analysis instructions, and output format</li>' +
            '<li>The <strong>User Prompt Template</strong> provides the data and analysis request with placeholders</li></ol>' +
            '<p>Leave fields empty to use the built-in defaults. Prompts are stored in MongoDB.</p>',
          ...
          $formContainer.append(
              $('<div>').css('margin-bottom', '15px').append(
                  $('<label>').attr('for', systemPromptId).css({display: 'block', marginBottom: '5px'}).html(client.translate('<h3>System Prompt:</h3>' +
                      'Available tokens: {{LANGUAGE}}, {{DATEFORMAT}}, {{RETURNFORMAT}}, {{PROFILE_JSON}}')),
                  $('<textarea>').attr('id', systemPromptId).attr('rows', 8).css({width: '90%', fontFamily: 'monospace'}).attr('placeholder', client.translate('Leave empty to use built-in default. Defines the LLM role and analysis instructions.'))
              )
          );

          $formContainer.append(
              $('<div>').css('margin-bottom', '15px').append(
                  $('<label>').attr('for', userPromptTemplateId).css({display: 'block', marginBottom: '5px'}).html(client.translate('<h3>User Prompt Template:</h3>' +
                      'Available tokens: {{CGMDATA_JSON}}, {{STATS_JSON}}, {{PROFILE_JSON}}, {{TREATMENT_SUMMARY}}, ' +
                      '{{LANGUAGE}}, {{DATEFORMAT}}, {{RETURNFORMAT}}, {{TIMEFROM}}, {{TIMETILL}}, {{DAYS}}')),
                  $('<textarea>').attr('id', userPromptTemplateId).attr('rows', 12).css({width: '90%', fontFamily: 'monospace'}).attr('placeholder', client.translate('Leave empty to use built-in default. The analysis request sent to the LLM with data placeholders.'))
              )
          );

          $formContainer.append(
              $('<p>').append($('<em>').text(client.translate('Note: Tokens like {{CGMDATA_JSON}}, {{STATS_JSON}}, {{PROFILE_JSON}} etc. are replaced with actual data when the analysis runs. Leave both fields empty to use the built-in default prompts.')))
          );
        ...
        code: function(client, buttonElement) {
          ...
          const systemPromptId = `ai_system_prompt_${plugin.name}`;
          const userPromptTemplateId = `ai_user_prompt_template_${plugin.name}`;
          const systemPrompt = String($('#' + systemPromptId).val() || '');
          const userPromptTemplate = String($('#' + userPromptTemplateId).val() || '');
