...
function init(_ctx) {
    ...
    // XSS-safe HTML escaping for user/API-sourced values
    function esc (s) {
    ...
    const renderUsageTable = (client, placeholderId, data) => {
        ...
        if (exchangeRateInfo) {
            costHeaders += `<th class="exchange-rate exchange-rate-foreign" colspan="3">${client.translate('Costs')} (${esc(exchangeRateInfo.currency)})</th>`;
        }
    ...
    `;
      ...
      `;
        ...
        `;
            ...
            tableHtml += `
        <tr>
          <td>${esc(monthEntry.month)}</td>
          <td>${esc(monthEntry.requests)}</td>
          <td>${esc(monthEntry.total_days_requested)}</td>
          <td>${parseFloat(monthEntry.avg_days_per_request).toFixed(2)}</td>

          <td>${esc(monthEntry.total_repair_calls || 0)}</td>
          <td>${parseFloat(monthEntry.avg_repair_calls_per_request || 0).toFixed(2)}</td>

          <td>${esc(monthEntry.total_prompt_tokens)}</td>
          <td>${esc(monthEntry.total_completion_tokens)}</td>
          <td>${esc(monthEntry.total_tokens)}</td>
      ...
      `;
        ...
        if (exchangeRateInfo) {
            tableHtml += `<p><em>${client.translate('Currency conversion is activated via AI_LLM_EXCHANGERATE_API_CURRENCY - make sure AI_LLM_EXCHANGERATE_API_KEY is set. Exchange rate (1 USD = %2 %1)', { params: [esc(exchangeRateInfo.currency), esc(exchangeRateInfo.rate.toFixed(4))] })}</em></p>`;
