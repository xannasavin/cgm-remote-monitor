...
function configure (app, wares, ctx, env) {
  ...
  // Route-level parser with an explicit 10mb limit. The previous
  // `wares.bodyParser()` call here was a no-op (body-parser is an object with
  // .json/.urlencoded/.raw methods, not a callable factory) — body parsing was
  // silently falling through to the global parser in app.js. This makes the
  // parser and its limit explicit at the route boundary.
  api.post('/', wares.bodyParser.json({ limit: '10mb' }), ctx.authorization.isPermitted('api:treatments:read'), function (req, res) {
