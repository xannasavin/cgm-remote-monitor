...
// Field whitelist: only forward known-safe fields to the LLM API (Finding #2)
var ALLOWED_FIELDS = ['messages', 'model', 'temperature', 'top_p', 'max_tokens', 'response_format'];
  ...
  api.post('/', wares.bodyParser(), ctx.authorization.isPermitted('api:treatments:read'), function (req, res) {
    ...
    request(requestOptions, function (error, response, body) {
      if (error) {
        ...
        var errMsg = error.code === 'ESOCKETTIMEDOUT' || error.code === 'ETIMEDOUT'
          ? 'LLM API request timed out after ' + (timeoutMs / 1000) + 's.'
          : 'Failed to connect to LLM API.';
        return res.status(500).json({ error: errMsg, details: error.message });
      }
