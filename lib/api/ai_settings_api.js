...
function configure(app, wares, ctx) {
  ...
  api.post('/prompts', ctx.authorization.isPermitted('admin:api:ai_settings:edit'), async (req, res) => {
    ...
    try {
      ...
      // A successful upsert will have acknowledged=true AND either a modifiedCount=1 or an upsertedCount=1 (with an upsertedId)
      if (result.acknowledged && (result.modifiedCount === 1 || result.upsertedCount === 1)) {
        if (result.upsertedCount === 1) {
          console.log('[AISettingsAPI POST /prompts] Prompts document created (upserted). ID:', result.upsertedId);
        } else {
          console.log('[AISettingsAPI POST /prompts] Prompts document updated. Matched:', result.matchedCount, 'Modified:', result.modifiedCount);
        }
        ...
        console.warn('[AISettingsAPI POST /prompts] Database update did not behave as expected or was not acknowledged. Result:', result);
        if (!result.acknowledged) {
          throw new Error('Database update not acknowledged by the server.');
        } else if (result.matchedCount === 0 && result.upsertedCount === 0) {
          // This implies the document with _id: CONFIG_ID was somehow not found and not upserted.
          // Should not happen with upsert:true unless there's a very strange issue.
          throw new Error('Database update failed: Document not found and not upserted.');
        } else {
          // General case if modified/upserted counts are not 1 but it was acknowledged.
          throw new Error('Database update acknowledged but did not modify or upsert the expected document.');
        }
      }
    } catch (error) {
