...
function configure(app, wares, ctx) {
  ...
  api.get('/prompts', ctx.authorization.isPermitted('api:treatments:read'), async (req, res) => { // Using 'api:treatments:read' for now for GET, admin for POST
    try {
      ...
      console.log('[AISettingsAPI GET /prompts] Attempting to access ctx.store. typeof ctx.store:', typeof ctx.store);
      if (ctx && typeof ctx.store !== 'function') {
        console.error('[AISettingsAPI GET /prompts] ctx.store is NOT a function. Current ctx keys:', Object.keys(ctx).join(', '));
        // For more detailed debugging, you might log the entire ctx object, but be wary of large objects in logs.
        // console.log('[AISettingsAPI GET /prompts] Full ctx object:', ctx);
      }
