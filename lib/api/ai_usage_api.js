...
async function getExchangeRate(ctx, env) {
  // F13: Read exchange rate settings from env, not ctx.settings (which has no defaults)
  const targetCurrency = env.ai_llm_exchangerate_api_currency;
  if (!targetCurrency) {
    return null;
  }
  // F18: Validate currency code format
  if (!/^[A-Z]{3}$/.test(targetCurrency)) {
    console.error('AI_LLM_EXCHANGERATE_API_CURRENCY must be a 3-letter ISO code, got:', targetCurrency);
    return null;
  }
  ...
  const pollingIntervalDays = env.ai_llm_exchangerate_api_polling_interval || 7;
  const monthlyLimit = env.ai_llm_exchangerate_api_limit || 100;
  ...
  if (needsFetch && usageCount < monthlyLimit) {
    return new Promise(function (resolve) {
      // F17: Use Node.js built-in https instead of deprecated request package
      const queryParams = 'from=USD&to=' + encodeURIComponent(targetCurrency) + '&amount=1&access_key=' + encodeURIComponent(apiKey);
      const fetchUrl = 'https://api.exchangerate.host/convert?' + queryParams;
          ...
          try {
            ...
            if (body && body.success) {
              ...
              if (rate) {
                ...
                exchangeRatesCollection.findOneAndUpdate(
                  { base_currency: 'USD', target_currency: targetCurrency },
                  { $set: newRateRecord },
                  { upsert: true }
                ).then(function () {
                  resolve({ rate: rate, currency: targetCurrency });
                }).catch(function (err) {
                  console.error('Error saving exchange rate to database:', err);
                  console.warn('Exchange rate fetched but NOT persisted. Will retry on next request.');
                  resolve({ rate: rate, currency: targetCurrency, persistenceError: true });
                });
                return;
              }
            }
            ...
            console.error('Error parsing exchange rate response:', e.message);
            if (lastRate) return resolve({ rate: lastRate.rate, currency: lastRate.target_currency });
            resolve(null);
          }
        });
      ...
      req.on('timeout', function () {
        req.destroy();
        console.error('Exchange rate request timed out.');
        if (lastRate) return resolve({ rate: lastRate.rate, currency: lastRate.target_currency });
        resolve(null);
      });
  ...
  if (lastRate) return { rate: lastRate.rate, currency: lastRate.target_currency };
  return null;
}
...
function configure(app, wares, ctx, env) {
  ...
  api.post('/record', ctx.authorization.isPermitted('admin:api:ai_usage:edit'), async (req, res) => {
    ...
    try {
      ...
      res.sendJSONStatus(res, 500, 'Error recording AI usage', 'Internal server error');
    }
  });
  ...
  api.get('/check_limit', ctx.authorization.isPermitted('api:treatments:read'), async (req, res) => {
    try {
      ...
      res.sendJSONStatus(res, 500, 'Error checking AI usage limit', 'Internal server error');
    }
  });
...
async function rebuildSummary(ctx) {
    ...
    // F26: Use upsert per document instead of delete-all + insert to prevent data loss on crash
    ...
    // Upsert each month individually (crash-safe)
    for (let mi = 0; mi < monthlyData.length; mi++) {
        const md = monthlyData[mi];
        await summaryCollection.replaceOne({ _id: md._id }, md, { upsert: true });
    }
    if (totalData.length > 0) {
        await summaryCollection.replaceOne({ _id: 'all_time' }, totalData[0], { upsert: true });
    }
    // Remove stale months no longer in raw data
    const validIds = monthlyData.map(function (m) { return m._id; }).concat(['all_time']);
    await summaryCollection.deleteMany({ _id: { $nin: validIds } });
}

  api.post('/rebuild_summary', ctx.authorization.isPermitted('admin:api:ai_usage:edit'), async (req, res) => {
    try {
        ...
        res.sendJSONStatus(res, 500, 'Error rebuilding AI usage summary', 'Internal server error');
    }
  });

  api.post('/delete_old', ctx.authorization.isPermitted('admin:api:ai_usage:edit'), async (req, res) => {
    try {
        ...
        res.sendJSONStatus(res, 500, 'Error deleting old AI usage data', 'Internal server error');
    }
  });
  ...
  api.get('/monthly_summary', ctx.authorization.isPermitted('api:treatments:read'), async (req, res) => {
    try {
      ...
      res.sendJSONStatus(res, 500, 'Error fetching AI usage summary', 'Internal server error');
    }
  });
