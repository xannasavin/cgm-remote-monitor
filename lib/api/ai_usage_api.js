...
async function getExchangeRate(ctx, env) {
  ...
  const pollingIntervalDays = ctx.settings.ai_llm_exchangerate_api_polling_interval || ctx.settings.ai_llm_exchangerate_api_poling_intervall || 7;
      ...
      request(requestOptions, function (error, response, body) {
        ...
        if (body && body.success) {
          ...
          if (rate) {
            ...
            // Use findOneAndUpdate with upsert keyed on currency pair (Finding #21)
            exchangeRatesCollection.findOneAndUpdate(
              ...
              // Still return the fetched rate (it's valid), but warn about persistence failure
