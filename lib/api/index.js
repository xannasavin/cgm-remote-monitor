...
function create (env, ctx) {
  ...
  // AI APIs: only register when AI_LLM_KEY is set (Step 3.4 plugin isolation)
  if (env.settings.ai_llm_key_is_set) {
    app.use('/ai_settings', require('./ai_settings_api')(app, wares, ctx));
    app.use('/ai_usage', require('./ai_usage_api')(app, wares, ctx, env));
    app.use('/ai_eval', require('./ai_eval_api')(app, wares, ctx, env));
  }
