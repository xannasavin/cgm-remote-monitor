...
function init (env, callback) {
  ...
  function loadData (path) {
    ...
    try {
      purgeCache(path);
      // eslint-disable-next-line security/detect-non-literal-require -- path from trusted operator OPENAPS_REPORT config
      ...
      if (_.isArray(inputData)) {
        ...
        // eslint-disable-next-line security/detect-non-literal-fs-filename -- path from trusted operator OPENAPS_REPORT config
  ...
  try {
    // eslint-disable-next-line security/detect-non-literal-require -- configPath from trusted operator CUSTOM_CONFIG env var
