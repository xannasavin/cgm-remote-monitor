...
// Render a treatment-insight list where items can be strings (legacy shape)
// or { text, refers_to_hotspot? } objects (current schema). Items with a
// refers_to_hotspot citation get a "hot" data-attribute so Phase D can
// visually link them back to the hotspot chart; items without a citation
// that mention basal/temp-basal/hotspot language get an "AI inference" hint.
//
// R1-12: the title/tooltip now lives on the badge <span> itself, not on the
// enclosing <li>, so hovering the badge triggers the tooltip.
// R2-12: ref.hour is range-checked to 0–23 before we honor it.
// R3-5: non-string `text` (null, number, undefined) is normalized to '' and
//       logged as a schema-violation warning instead of silently coercing.
// R2-5: when a `validHotspots` Set is provided, the citation is only honored
//       if the cited hour+signal actually exists in the computed hotspots.
var BASAL_LANG_RE = /basal|temp basal|hotspot|suspension/i;

function coerceText (raw) {
  if (typeof raw === 'string') return raw;
  if (raw == null) return '';
  // Non-string text violates the schema — warn once so devs notice.
...
function inferredBadge () {
  return ' <span class="cgm-grounding-badge" title="'
    + escapeHtml(translate('AI inference — not linked to a computed hotspot'))
    + '">?</span>';
}

function linkedBadge (hour) {
  return ' <span class="cgm-grounding-badge cgm-grounding-linked" title="'
    + escapeHtml(translate('Cites computed hotspot at hour') + ' ' + hour)
    + '">\u2039\u2192\u203a</span>';
}

function renderInsightItem (item, validHotspots) {
  if (item == null) return '';
  var text;
  var ref;
  if (typeof item === 'string') {
    ...
    ref = item.refers_to_hotspot;
  } else {
    return '';
  }
  if (!text) return '';

  var hourValid = ref && typeof ref.hour === 'number' && ref.hour >= 0 && ref.hour <= 23;
  var signalValid = hourValid && typeof ref.signal === 'string';
  var citationExists = !validHotspots
    || (signalValid && validHotspots.has(ref.hour + '|' + ref.signal))
    || (signalValid && validHotspots.has(String(ref.hour))); // fallback for renderer with only hours
  if (hourValid && signalValid && citationExists) {
    return '<li data-grounding="hotspot" data-hotspot-hour="' + escapeHtml(String(ref.hour)) + '"'
      + ' data-hotspot-signal="' + escapeHtml(ref.signal) + '">'
      + escapeHtml(text)
      + linkedBadge(String(ref.hour))
      + '</li>';
  }
  var inferred = BASAL_LANG_RE.test(text);
  return '<li' + (inferred ? ' data-grounding="inferred"' : '') + '>'
    + escapeHtml(text)
    + (inferred ? inferredBadge() : '')
    + '</li>';
}

function renderInsightList (items, validHotspots) {
  ...
  return '<ul>' + items.map(function (item) { return renderInsightItem(item, validHotspots); }).join('') + '</ul>';
}
...
function renderStats (periodStats, dayStats, meta, dayDates, pumpActionStats, episodeTiming) {
  ...
  // R4-3: Daily Breakdown is the most useful table for spotting day-to-day
  // trends — keep it open by default. Treatment Summary and Diurnal Patterns
  // stay collapsed to cut visual overwhelm.
  return '<div class="cgm-wrap">'
    ...
    + (diurnalHtml ? '<details class="cgm-details"><summary>' + escapeHtml(translate('Diurnal Patterns')) + '</summary>' + chartPlaceholder('aiDiurnalChart', true) + diurnalHtml + '</details>' : '')
    + '<details class="cgm-details" open><summary>' + escapeHtml(translate('Daily Breakdown')) + '</summary>'
...
// Build the set of valid "hour|signal" hotspot citations the LLM is allowed
// to link to. R2-5: this guards against the LLM hallucinating hotspots that
// don't exist in the computed output — those will fall back to the "AI
// inference" badge rather than rendering a broken link.
function buildValidHotspotSet (pumpActionStats) {
  if (!pumpActionStats || !pumpActionStats.hotspots || !Array.isArray(pumpActionStats.hotspots.perHour)) {
    return null;
  }
  var set = new Set();
  pumpActionStats.hotspots.perHour.forEach(function (p) {
    if (!p || typeof p.hour !== 'number') return;
    var signals = p.signals || {};
    if ((signals.tempBasalDaysUp || 0) > 0 || (signals.tempBasalDaysDown || 0) > 0) {
      set.add(p.hour + '|temp_basal_delta');
    }
    if ((signals.autoBolusDays || 0) > 0) set.add(p.hour + '|auto_bolus');
    if ((signals.suspensionDays || 0) > 0) set.add(p.hour + '|basal_suspension');
  });
  return set;
}

function renderAnalysis (d, pumpActionStats) {
  if (!d) return '<p>No AI analysis available.</p>';

  var validHotspots = buildValidHotspotSet(pumpActionStats);
  ...
  // Items may arrive as plain strings (legacy v1 schema) or as objects of
  // shape { text, refers_to_hotspot? } (v1.1+ schema). Normalize to a
  // rendered list with an optional grounding badge.
  ...
  if (d.treatment_insights) {
    ...
    treatmentInsightsHtml = '<section><h2>' + escapeHtml(translate('Treatment Insights')) + '</h2>'
      ...
      + '<section><h3>' + escapeHtml(translate('Carb Patterns')) + '</h3>' + renderInsightList(ti.carb_patterns, validHotspots) + '</section>'
      + '<section><h3>' + escapeHtml(translate('Insulin Patterns')) + '</h3>' + renderInsightList(ti.insulin_patterns, validHotspots) + '</section>'
      + '<section><h3>' + escapeHtml(translate('Basal Observations')) + '</h3>' + renderInsightList(ti.basal_observations, validHotspots) + '</section>'
      + '<section><h3>' + escapeHtml(translate('Dosing Observations')) + '</h3>' + renderInsightList(ti.dosing_observations, validHotspots) + '</section>'
      + '</div></section>';
  }
