...
function renderPumpActionHotspot (containerId, data, options) {
  ...
  var margin = { top: 18, right: 24, bottom: 32, left: 50 };
  var width = 900 - margin.left - margin.right;
  var height = 140 - margin.top - margin.bottom;

  var svg = d3.select(containerId).append('svg')
    .attr('viewBox', '0 0 900 140')
    .attr('preserveAspectRatio', 'xMidYMid meet')
    .attr('role', 'img')
    // R1-4: aria-label mentions the episode overlay dots
  ...
  for (var i = 0; i < perHour.length; i++) {
    ...
    var t = p.intensity || 0;
    var fill;
    if (t < 0.05) fill = COLORS.neutral;
    else if ((p.direction || 0) >= 0) fill = 'rgba(' + COLORS.directionUpBase + ', ' + (0.15 + t * 0.7).toFixed(3) + ')';
    else fill = 'rgba(' + COLORS.directionDownBase + ', ' + (0.15 + t * 0.7).toFixed(3) + ')';

    g.append('rect')
      .attr('x', x(p.hour)).attr('y', 0)
      .attr('width', cellW - 1).attr('height', height)
      .attr('fill', fill)
      .attr('stroke', hotspots.indexOf(p.hour) !== -1 ? COLORS.hotspotOutline : 'none')
      .attr('stroke-width', 2);
  ...
  // R1-4: overlay circles get hover tooltips + the chart's aria-label
  // already mentions the episode markers, below.
    ...
    for (var h = 0; h < 24; h++) {
      if ((ep.hypoHourly && ep.hypoHourly[h] > 0)) {
        var hypoDot = g.append('circle')
          .attr('cx', x(h) + cellW / 2 - 5).attr('cy', height + 8)
          .attr('r', 3).attr('fill', COLORS.hypo).attr('opacity', 0.9);
        hypoDot.append('title').text(translate('Hypo episode marker'));
      }
      if ((ep.hyperHourly && ep.hyperHourly[h] > 0)) {
        var hyperDot = g.append('circle')
          .attr('cx', x(h) + cellW / 2 + 5).attr('cy', height + 8)
          .attr('r', 3).attr('fill', COLORS.hyper).attr('opacity', 0.9);
...
function renderEpisodeTimingChart (containerId, data) {
  ...
  // R1-6: in-chart legend so users don't infer left/right split from colors alone
  var legend = svg.append('g').attr('transform', 'translate(' + (margin.left + width - 260) + ',' + (margin.top - 4) + ')');
