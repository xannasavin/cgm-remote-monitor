                    ...
                    if (reportData && reportData.datastorage) {
                        ...
                        if (reportData.datastorage) {
                            ...
                            for (const [key, value] of Object.entries(datastorageAltered)) {
                                ...
                                day.date = key;
                                day.dailyCarbs = key.dailyCarbs;
                                day.treatments = [];

                                if(key.treatments.length() > 0) {

                                    let treatment = {};

                                    for (let i = 0; i < key.treatments.length; i++) {
                                        //console.log(key.treatments[i]);

                                        const keys = ['mills', 'carbs', 'insulin', 'notes'];
                                        const source = key.treatments[i];
                                ...
                                if(key.sgv.length() > 0) {
                                    ...
                                    for (let i = 0; i < key.sgv.length; i++) {
                                        //console.log(key.treatments[i]);

                                        const keys = ['mills', 'sgv'];
                                        const source = key.svg[i];
