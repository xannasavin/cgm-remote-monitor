...
function init(ctx) {
    ...
    function initializeAiEvalTab(passedInClient) {
        ...
        console.log('AI Eval: initializeAiEvalTab called. Received client:', passedInClient);
        // Log the state of tempAiEvalReportData as soon as the function is entered
        if (typeof window !== 'undefined') {
            console.log('AI Eval: Initial check of window.tempAiEvalReportData in initializeAiEvalTab:', window.tempAiEvalReportData);
        } else {
            console.log('AI Eval: window object not found at start of initializeAiEvalTab');
        }
        ...
        if (typeof window !== 'undefined' && window.tempAiEvalReportData) {
            ...
            console.warn('AI Eval: window.tempAiEvalReportData not found during retrieval phase in initializeAiEvalTab. Payload construction will be limited.');
            const debugArea = document.getElementById('aiEvalDebugArea');
            if (debugArea) {
                debugArea.textContent = 'DEBUG: `initializeAiEvalTab` ran, but `window.tempAiEvalReportData` was not found or was undefined. Ensure the AI Evaluation report\'s "Show" button triggers data loading before this script runs.';
            }
        }
