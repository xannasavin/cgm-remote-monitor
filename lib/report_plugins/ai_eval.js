...
function init(ctx) {
  ...
  function initializeAiEvalTab(passedInClient) {
    ...
    // This is the flag we EXPECT the server to set. It will be undefined for now.
    const apiKeyIsConfiguredByServer = settings.ai_llm_key_is_configured;
    ...
    // API Key -  status now depends on the server-provided 'ai_llm_key_is_configured' flag
    let apiKeySetText, apiKeyValueClass;
    if (apiKeyIsConfiguredByServer === true) {
      apiKeySetText = 'Set';
      apiKeyValueClass = 'ai-setting-value-set';
    } else if (apiKeyIsConfiguredByServer === false) {
      apiKeySetText = 'Not Set';
      apiKeyValueClass = 'ai-setting-value-not-set';
    } else { // undefined or any other value
      apiKeySetText = 'Unknown'; // Correctly reflects that client doesn't know without server flag
      apiKeyValueClass = 'ai-setting-value-not-set';
    }
    statusHTML += '<div><span class="ai-setting-label">API Key Configured: </span><span class="' + apiKeyValueClass + '">' + apiKeySetText + '</span></div>';
    statusHTML += '<div><span style="font-size:0.8em; color: #666;"><em>(API Key status requires a server-side update to be accurate)</em></span></div>';
