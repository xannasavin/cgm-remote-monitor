    ...
    function initializeAiEvalTab(passedInClient) {
        ...
        if (sendButton) {
            sendButton.addEventListener('click', function () {
                ...
                const payloadToSend = window.interimPayloads[0]; // Send the first interim payload
                ...
                const interimResponseDebugArea = document.getElementById('aiEvalInterimResponseDebugArea');
                if (interimResponseDebugArea) {
                    interimResponseDebugArea.textContent = 'Calling API for interim payload 1/' + window.interimPayloads.length + '...';
                }
                ...
                fetch(apiEndpoint, {
                    ...
                    .then(response => {
                        console.log('AI Eval: Received initial response from server for /api/v1/ai_eval');
                        if (!response.ok) {
                        ...
                        interimResponseDebugArea.textContent = 'Receiving streaming response...\n\n';

                        function processText({ done, value }) {
                            if (done) {
                                console.log('AI Eval: Stream complete.');
                            ...
                            // In debug mode, show the raw streaming chunks as they arrive
                            if (passedInClient.settings.ai_llm_debug === true) {
                                interimResponseDebugArea.textContent += chunk;
                            } else {
                                // For non-debug, you might parse the stream if it's structured (e.g., JSON chunks)
                                // For now, just showing a progress message.
                                interimResponseDebugArea.textContent = 'Processing...';
                            }
                    ...
                    .catch(error => {
                        ...
                        button.disabled = false;
                        if (interimResponseDebugArea) {
                            interimResponseDebugArea.style.display = 'block';
                            interimResponseDebugArea.textContent = 'AI API Call Error:\n\n' + error.message;
                        } else {
            ...
            $.ajax({
                ...
                success: function (prompts) {
                    ...
                    $('#ai-system-prompt-status')
                        .text('Waiting for interim calls...')
                        .removeClass('ai-setting-value-loading')
                        .addClass('ai-setting-value-loading');
                    ...
                    if (reportData && reportData.datastorage) {
                        ...
                        if (reportData.datastorage) {
                            ...
                            for (let i = 0; i < cgmData.days.length; i++) {
                                ...
                                if (settings.ai_llm_debug === true) {
                                    //console.log('AI Eval DEBUG: Interim Payloads Markdown for current Day: ', generateMarkdownFromDay(cgmData.days[i]));
                                    //console.log('AI Eval DEBUG: Interim Payloads tempUserPromptContent for current Day: ', tempUserPromptContent);
                                }
                        ...
                        // Make the finalPayload available for the send button
                        if (typeof window !== 'undefined') {
                            window.interimPayloads = interimPayloads;
                        }

                        if (settings.ai_llm_debug === true) {
                            //console.log('AI Eval: Constructed finalPayload:', finalPayload);

                            const debugAreaInterim = document.getElementById('aiEvalInterimDebugArea');
                            if (debugAreaInterim) {
                                debugAreaInterim.textContent = 'INTERIM PAYLOADS (DEBUG):\n\n' + debugInterimPayload;
                                console.log('AI Eval: Interim Payload displayed in debug area.');
                            }

                            const debugArea = document.getElementById('aiEvalDebugArea');
                            if (debugArea) {
                                debugArea.textContent = 'FINAL PAYLOAD (DEBUG):\n\nNot created yet. Will be created after the interim calls, when you click on "Send to AI".';
                                //console.log('AI Eval: finalPayload displayed in debug area.');
                            }
                        }
                    } else {
