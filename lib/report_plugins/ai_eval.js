...
function init(ctx) {
    ...
    function initializeAiEvalTab(passedInClient) {
        ...
        const responseInterimDebugArea = document.getElementById('aiEvalInterimResponseDebugArea');
        if (responseInterimDebugArea && settings.ai_llm_debug === true) {
            responseInterimDebugArea.style.display = 'block';
            responseInterimDebugArea.textContent = 'AI Interim Response Debug Area: Waiting for AI call...';
        } else if (responseInterimDebugArea) {
            responseInterimDebugArea.style.display = 'none';
        }
        ...
        if (sendButton) {
            sendButton.addEventListener('click', function () {
                ...
                if (typeof window === 'undefined' || !window.currentAiEvalfinalPayload) {
                ...
                const payloadToSend = window.currentAiEvalfinalPayload;
                ...
                const responseInterimDebugArea = document.getElementById('aiEvalInterimResponseDebugArea');
                if (responseInterimDebugArea && passedInClient.settings.ai_llm_debug === true) {
                    responseInterimDebugArea.textContent = 'Calling API...';
                }
            ...
            $.ajax({
                ...
                success: function (prompts) {
                    ...
                    if (reportData && reportData.datastorage) {
                        ...
                        const systemInterimPromptContent = prompts.system_interim_prompt || "You are a helpful assistant.";
                        let userInterimPromptContent = prompts.user_interim_prompt_template || "Here is the CGM Data for one day: {{CGMDATA}} Analyze and summarize it in a way, so the return can be combined with your return of other days to analyze the whole timeframe.  This is the profile for this day: {{PROFILE}}";
                        ...
                        if (reportData.datastorage) {
                            ...
                            cgmData.profiles = formatProfileMarkdown(datastorageAltered.profiles[0]);
                            ...
                            // systemInterimPromptContent
                            // userInterimPromptContent
                            ...
                            function generateMarkdownFromDay(day) {
                                ...
                                const dateFormatted = `${String(startDate.getDate()).padStart(2, '0')}.${String(startDate.getMonth() + 1).padStart(2, '0')}.${startDate.getFullYear()}`;
                                const store = day.store[day.defaultProfile];
                                ...
                                return [
                                    `# Profil aktiv ab: ${dateFormatted}\n`,
                                    `**Einheit für Blutzuckerwerte:** ${day.units}\n`,
                                    section('Basalrate (IE/h)', store.basal),
                                    section('Carbratio (g/IE)', store.carbratio),
                                    section('Insulinempfindlichkeit (mg/dl pro IE)', store.sens),
                                    rangeSection('Zielbereich', store.target_low, store.target_high)
                            ...
                            let interimPayloads = [];
                            userInterimPromptContent = userInterimPromptContent.replace('{{PROFILE}}', cgmData.profiles);
                            ...
                            if (settings.ai_llm_debug === true) {
                                console.log('AI Eval DEBUG: =================================');
                                console.log('AI Eval DEBUG: cgmDataString: ', interimPayloads);
                                console.log('AI Eval DEBUG: =================================');
                            }

                            //interimPayload FOR DEBUGGING AREA
                            ...
                            cgmDataString += generateMarkdownFromDays(cgmData.days);
                        ...
                        userPromptContent = userPromptContent.replace('{{PROFILE}}', cgmData.profiles);
