    ...
    function initializeAiEvalTab(passedInClient) {
        ...
        if (sendButton) {
            sendButton.addEventListener('click', function () {
                ...
                // For now, we only process the first interim payload
                const payloadToSend = window.interimPayloads[0];
                ...
                console.log('AI Eval: Calling AI API for interim payload at URL:', apiEndpoint);
                if (passedInClient.settings.ai_llm_debug === true) {
                    console.log('AI Eval: Interim payload being sent:', JSON.stringify(payloadToSend, null, 2));
                }

                button.textContent = 'Sending Interim Request (1/1)...';
                button.disabled = true;

                const interimResponseDebugArea = document.getElementById('aiEvalInterimResponseDebugArea');
                if (interimResponseDebugArea) {
                    interimResponseDebugArea.textContent = 'Calling API with first interim payload...';
                }
                ...
                .then(response => {
                    console.log('AI Eval: Received response from server for interim call');
                    if (!response.ok) {
                ...
                .then(data => {
                    console.log('AI Eval: Interim AI API call successful. Response data:', data);
                    button.textContent = 'Send to AI'; // Reset button for now
                    button.disabled = false; // Re-enable for now

                    if (typeof window !== 'undefined') {
                        window.interimResponses = window.interimResponses || [];
                    ...
                    // Further steps (like processing all interim payloads and then the final one) will be implemented next.
                })
                .catch(error => {
                    console.error('AI Eval: Interim AI API call failed:', error);
                    button.textContent = 'Send to AI'; // Reset button
                    button.disabled = false;
                    if (interimResponseDebugArea) {
                        interimResponseDebugArea.textContent = 'Interim AI API Call Error:\n\n' + error.message;
                    } else {
                        alert('Interim AI API Call Error: ' + error.message);
                    }
                });
            });
        }
                    ...
                    if (reportData && reportData.datastorage) {
                        ...
                        if (reportData.datastorage) {
                            ...
                            for (let i = 0; i < cgmData.days.length; i++) {
                                ...
                                if (typeof window !== 'undefined') {
                                    window.interimPayloads.push(interimPayload);
                                }
                            }
