            ...
            sendButton.addEventListener('click', async function () {
                ...
                $.ajax({
                    ...
                    success: function (prompts) {
                        ...
                        const reformatedInterimResponses = {
                            merged_by_date: mergedByDate,
                            total_tokens_used: totalTokensUsed,
                            interim_calls_amount: interim_calls_amount,
                            date_from: date_from,
                            date_till: date_till
                        };

                        if (passedInClient.settings.ai_llm_debug === true) {
                            console.log('AI Eval Debug: new json from interim api responses:', reformatedInterimResponses);
                        }

                        const interimDataString = JSON.stringify(reformatedInterimResponses.merged_by_date, null, 2);
                        finalUserPrompt = finalUserPrompt.replace('{{INTERIMAIDATA}}', interimDataString);
                        finalUserPrompt = finalUserPrompt.replace('{{TIMEFROM}}', reformatedInterimResponses.date_from);
                        finalUserPrompt = finalUserPrompt.replace('{{TIMETILL}}', reformatedInterimResponses.date_till);
                        finalUserPrompt = finalUserPrompt.replace('{{DAYS}}', reformatedInterimResponses.interim_calls_amount);
