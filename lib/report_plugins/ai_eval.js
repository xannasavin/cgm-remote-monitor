            ...
            $.ajax({
                ...
                success: function (prompts) {
                    ...
                    if (reportData && reportData.datastorage) {
                        ...
                        let cgmDataString = "CGM data not available or datastorage.data is missing.";
                        if (reportData.datastorage.data) {
                            const relevantCGMData = {
                                ...
                                // Profile data is part of CGM data here, but a specific {{PROFILE}} token will get a more focused version.
                                profileContext: reportData.datastorage.data.profile || reportData.datastorage.profile,
                                deviceStatus: reportData.datastorage.data.devicestatus
                            };

                            if (settings.ai_llm_debug === true) {
                                console.log('AI Eval DEBUG: =================================');
                                console.log('AI Eval DEBUG: ENTRIES: ', reportData.datastorage.data.entries);
                                console.log('AI Eval DEBUG: ENTRIES stringified: ', JSON.stringify(reportData.datastorage.data.entries, null, 2));
                                console.log('AI Eval DEBUG: treatments: ', reportData.datastorage.data.treatments);
                                console.log('AI Eval DEBUG: profileContext: ', reportData.datastorage.data.profile || reportData.datastorage.profile);
                                console.log('AI Eval DEBUG: deviceStatus: ', reportData.datastorage.data.devicestatus);
                                console.log('AI Eval DEBUG: =================================');
                            }


                            /*
                            let cgmDataString = "CGM data not available.";
                            const relevantCGMData = {};
                            const cgmDataKeysToInclude = ['entries', 'treatments', 'devicestatus', 'profileSwitchTreatments', 'tempbasalTreatments', 'combobolusTreatments'];

                            if (reportData.datastorage) {
                                const ds_data = reportData.datastorage.data || {}; // Use .data if it exists
                                const ds_root = reportData.datastorage; // Always check root as well

                                cgmDataKeysToInclude.forEach(function(key) {
                                    let sourceObject = null;
                                    if (ds_data[key] && Array.isArray(ds_data[key]) && ds_data[key].length > 0) {
                                        sourceObject = ds_data;
                                    } else if (ds_root[key] && Array.isArray(ds_root[key]) && ds_root[key].length > 0) {
                                        sourceObject = ds_root;
                                    }

                                    if (sourceObject) {
                                        relevantCGMData[key] = sourceObject[key];
                                    }
                                });
                        ...
                        } else if (reportData.datastorage) {
                            cgmDataString = JSON.stringify(reportData.datastorage, null, 2); // Fallback
                            console.log('AI Eval DEBUG: =================================');
                            console.warn('AI Eval: datastorage.data was not found, stringified entire datastorage for CGMData placeholder.');
                            console.log('AI Eval DEBUG: datastorage: ', reportData.datastorage);
                            console.log('AI Eval DEBUG: datastorage stringified: ', JSON.stringify(reportData.datastorage, null, 2));
                            console.log('AI Eval DEBUG: =================================');

                            /*
                                console.log('AI Eval: Extracted keys for {{CGMDATA}}:', Object.keys(relevantCGMData));
                                } else {
                                    console.warn('AI Eval: No relevant CGM data arrays found in datastorage for keys:', cgmDataKeysToInclude.join(', '));
