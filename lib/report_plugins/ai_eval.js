                ...
                success: function (prompts) {
                    ...
                    if (reportData && reportData.datastorage) {
                        ...
                        let cgmDataForPayload = {
                            ...
                            treatments: "Treatments data not available.", // Placeholder, can be expanded if treatments also need reformatting
                            deviceStatus: "Device status data not available." // Placeholder
                        };
                        ...
                        if (reportData.datastorage && reportData.datastorage.data) {
                            ...
                            // Process Entries
                            if (dsData.entries && Array.isArray(dsData.entries)) {
                                let processedEntries = dsData.entries.map(entry => {
                                    // Ensure sgv is a number, default to null if not
                                    const sgvValue = (typeof entry.sgv === 'number' && !isNaN(entry.sgv)) ? entry.sgv : null;

                                    // Convert date to Unix timestamp (seconds)
                                    // Assumes entry.date is either ms since epoch or a string parsable by Date
                                    let unixTimestamp;
                                    if (typeof entry.date === 'string') {
                                        unixTimestamp = Math.floor(new Date(entry.date).getTime() / 1000);
                                    } else if (typeof entry.date === 'number') {
                                        unixTimestamp = Math.floor(entry.date / 1000); // Assuming it's already ms
                                    } else {
                                        unixTimestamp = null; // Or handle as an error
                                    }

                                    return { date: unixTimestamp, sgv: sgvValue };
                                }).filter(entry => entry.sgv !== null && entry.date !== null); // Filter out entries with null sgv or date after processing

                                // **Feasibility Check for 15-minute averaging:**
                                // For 2 weeks of data (5-min intervals): 14 days * 24 hours/day * 12 entries/hour = 4032 entries.
                                // Grouping and averaging this client-side is feasible.
                                if (processedEntries.length > 0) {
                                        ...
                                        if (entry.date < currentIntervalStart + fifteenMinutesInSeconds) {
                                            ...
                                            if (countSgv > 0) {
                                                averagedEntries.push({
                                                    date: currentIntervalStart, // Use the start of the interval as the timestamp
                                                    sgv: Math.round(sumSgv / countSgv)
                                                });
                                            }
                                            // Move to the next interval that contains the current entry
                                    ...
                                    if (settings.ai_llm_debug === true) {
                                        console.log('AI Eval: Entries averaged to 15-minute intervals. Count:', processedEntries.length);
                                    }
                                }
                                ...
                                if (settings.ai_llm_debug === true) {
                                    console.log('AI Eval: No entries data found or not an array.');
                                }
                            }

                            // Treatments (currently as-is, can be reformatted similarly if needed)
                            if (dsData.treatments) {
                                cgmDataForPayload.treatments = dsData.treatments; // Keep as is for now, or apply JSON lines if becomes large
                            }
                            // Device Status (currently as-is)
                            if (dsData.devicestatus) {
                                cgmDataForPayload.deviceStatus = dsData.devicestatus;
                            }

                        } else {
                            console.warn('AI Eval: reportData.datastorage.data was not found. CGM data will be marked as unavailable.');
                        }
