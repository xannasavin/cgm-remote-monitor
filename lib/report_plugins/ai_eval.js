...
function init(ctx) {
    ...
    function initializeAiEvalTab(passedInClient) {
        ...
        //console.log('AI Eval DEBUG Settings ai eval top', settings);
        ...
        if (settings.ai_llm_debug === true) {
            statusHTML += '<div><span class="ai-setting-label ai-setting-value-set">Debug Mode is active</span></div>';
        } else {
            statusHTML += '<div><span class="ai-setting-label">Debug Mode inactive</span></div>';
        }
        ...
        statusHTML += '<div><span class="ai-setting-label">Please make sure that "AI_LLM_KEY" Environment variable is set.</span></div>';
