            ...
            $.ajax({
                ...
                success: function (prompts) {
                    ...
                    if (reportData && reportData.datastorage) {
                        ...
                        let userPromptContent = prompts.user_prompt_template || "Analyze the following CGM data: {{CGMDATA}}";

                        let cgmDataString = "No CGM data loaded or datastorage.data is missing.";
                        if (reportData.datastorage.data) {
                            const relevantData = {
                                ...
                                profile: reportData.datastorage.data.profile || reportData.datastorage.profile,
                                deviceStatus: reportData.datastorage.data.devicestatus
                            };
                            cgmDataString = JSON.stringify(relevantData, null, 2);
                        } else if (reportData.datastorage) {
                            cgmDataString = JSON.stringify(reportData.datastorage, null, 2);
