            ...
            $.ajax({
                ...
                success: function (prompts) {
                    ...
                    if (reportData && reportData.datastorage) {
                        ...
                        if (reportData.datastorage) {
                            // Log datastorage.profiles (plural)
                            if (reportData.datastorage.profiles) {
                                console.log('AI Eval DEBUG: reportData.datastorage.profiles (raw):', JSON.stringify(reportData.datastorage.profiles, null, 2).substring(0, 500) + "...");
                                if (Array.isArray(reportData.datastorage.profiles) && reportData.datastorage.profiles.length > 0) {
                                    console.log('AI Eval DEBUG: reportData.datastorage.profiles[0]:', JSON.stringify(reportData.datastorage.profiles[0], null, 2).substring(0, 500) + "...");
                                    // Priority 1: Use datastorage.profiles[0]
                                    activeProfile = reportData.datastorage.profiles[0];
                                    profileSource = "datastorage.profiles[0]";
                                }
                            } else {
                                console.log('AI Eval DEBUG: reportData.datastorage.profiles is undefined or null.');
                            }
                        ...
                        if (!activeProfile && passedInClient) {
                            ...
                            console.log('AI Eval DEBUG: typeof passedInClient.profile:', typeof clientProfileFn);

                            if (typeof clientProfileFn === 'function') {
                                console.log('AI Eval: Attempting to get profile from passedInClient.profile() execution');
                                const clientProfileResult = clientProfileFn();
                                console.log('AI Eval DEBUG: passedInClient.profile() execution result (raw):', clientProfileResult);
                                console.log('AI Eval DEBUG: typeof clientProfileResult:', typeof clientProfileResult);

                                if (clientProfileResult && typeof clientProfileResult === 'object') {
                                    console.log('AI Eval DEBUG: Object.keys(clientProfileResult):', Object.keys(clientProfileResult));
                                    if (clientProfileResult.store && typeof clientProfileResult.store === 'object') {
                                        ...
                                        console.log('AI Eval DEBUG: using clientProfileResult.store:', JSON.stringify(activeProfile, null, 2).substring(0, 500) + "...");
                                    } else if (Object.keys(clientProfileResult).includes('basal')) { // Check for a key property like 'basal'
                                        activeProfile = clientProfileResult;
                                        profileSource = "passedInClient.profile() direct object";
                                        console.log('AI Eval DEBUG: using clientProfileResult object directly (has basal):', JSON.stringify(activeProfile, null, 2).substring(0, 500) + "...");
                                    }
                                }
                            } else if (typeof clientProfileFn === 'object' && clientProfileFn !== null) {
                                console.log('AI Eval DEBUG: passedInClient.profile is an object. Keys:', Object.keys(clientProfileFn));
                                if (clientProfileFn.store && typeof clientProfileFn.store === 'object') {
                                    ...
                                    console.log('AI Eval DEBUG: using passedInClient.profile.store (object access):', JSON.stringify(activeProfile, null, 2).substring(0, 500) + "...");
                                } else if (Object.keys(clientProfileFn).includes('basal')) { // Check for a key property
                                    activeProfile = clientProfileFn;
                                    profileSource = "passedInClient.profile direct object (has basal)";
                                    console.log('AI Eval DEBUG: using passedInClient.profile direct object (has basal):', JSON.stringify(activeProfile, null, 2).substring(0, 500) + "...");
                                }
                            }
                        }
