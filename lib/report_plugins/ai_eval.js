...
function init(ctx) {
            ...
            $.ajax({
                ...
                success: function (prompts) {
                    ...
                    if (reportData && reportData.datastorage) {
                        ...
                        let cgmDataForPayload = {
                            entries: "Entries data not available or could not be processed.",
                            treatments: "Treatments data not available.",
                            deviceStatus: "Device status data not available."
                        };

                        if (settings.ai_llm_debug === true) {
                            console.log('AI Eval DEBUG: Initial reportData.datastorage:', reportData.datastorage);
                            if (reportData.datastorage) {
                                console.log('AI Eval DEBUG: reportData.datastorage.data:', reportData.datastorage.data);
                            }
                        }

                        if (reportData.datastorage && reportData.datastorage.data) {
                            const dsData = reportData.datastorage.data;
                            if (settings.ai_llm_debug === true) {
                                console.log('AI Eval DEBUG: dsData (reportData.datastorage.data) is present.');
                                console.log('AI Eval DEBUG: dsData.entries type:', typeof dsData.entries, ', Is Array:', Array.isArray(dsData.entries));
                                if (Array.isArray(dsData.entries)) {
                                    console.log('AI Eval DEBUG: dsData.entries length:', dsData.entries.length);
                                    if (dsData.entries.length > 0) {
                                        console.log('AI Eval DEBUG: First original entry from dsData.entries:', dsData.entries[0]);
                                    }
                                }
                            }

                            // Process Entries
                            if (dsData.entries && Array.isArray(dsData.entries) && dsData.entries.length > 0) {
                                try {
                                    let processedEntries = dsData.entries.map((entry, index) => {
                                        const sgvValue = (typeof entry.sgv === 'number' && !isNaN(entry.sgv)) ? entry.sgv : null;
                                        let unixTimestamp;
                                        if (typeof entry.date === 'string') {
                                            unixTimestamp = Math.floor(new Date(entry.date).getTime() / 1000);
                                        } else if (typeof entry.date === 'number') {
                                            unixTimestamp = Math.floor(entry.date / 1000);
                                        } else {
                                            unixTimestamp = null;
                                        }

                                        // if (settings.ai_llm_debug === true && index < 3) { // Log first 3 transformations
                                        //     console.log(`AI Eval DEBUG: Mapping entry ${index}. Original:`, entry, `Processed: { date: ${unixTimestamp}, sgv: ${sgvValue} }`);
                                        // }
                                        return { date: unixTimestamp, sgv: sgvValue };
                                    }).filter(entry => entry.sgv !== null && entry.date !== null && !isNaN(entry.date));

                                    if (settings.ai_llm_debug === true) {
                                        console.log('AI Eval DEBUG: processedEntries after map/filter. Count:', processedEntries.length);
                                        if (processedEntries.length > 0) {
                                            console.log('AI Eval DEBUG: First entry after map/filter:', processedEntries[0]);
                                        }
                                    }
                                    ...
                                    if (processedEntries.length > 0) {
                                        ...
                                        let averagedEntries = [];
                                        // Sort by date just in case, crucial for interval logic
                                        processedEntries.sort((a, b) => a.date - b.date);

                                        let currentIntervalStart = Math.floor(processedEntries[0].date / fifteenMinutesInSeconds) * fifteenMinutesInSeconds;
                                        let sumSgv = 0;
                                        let countSgv = 0;

                                        for (let i = 0; i < processedEntries.length; i++) {
                                            const entry = processedEntries[i];
                                            // if (settings.ai_llm_debug === true && i < 5) { // Log first 5 entries into averaging
                                            //    console.log(`AI Eval DEBUG: Averaging loop ${i}. Entry date: ${entry.date}, sgv: ${entry.sgv}. Current interval: ${currentIntervalStart}`);
                                            // }
                                            if (entry.date < currentIntervalStart + fifteenMinutesInSeconds) {
                                                sumSgv += entry.sgv;
                                                countSgv++;
                                            } else {
                                                if (countSgv > 0) {
                                                    averagedEntries.push({
                                                        ...
                                                        sgv: Math.round(sumSgv / countSgv)
                                                    });
                                                }
                                                currentIntervalStart = Math.floor(entry.date / fifteenMinutesInSeconds) * fifteenMinutesInSeconds;
                                                sumSgv = entry.sgv;
                                                countSgv = 1;
                                            }
                                        }
                                        ...
                                        processedEntries = averagedEntries;
                                        if (settings.ai_llm_debug === true) {
                                            console.log('AI Eval DEBUG: Entries averaged. Count:', processedEntries.length);
                                            if (processedEntries.length > 0) {
                                                console.log('AI Eval DEBUG: First averaged entry:', processedEntries[0]);
                                            }
                                        }
                                    }

                                    if (processedEntries.length > 0) {
                                        cgmDataForPayload.entries = processedEntries.map(entry => JSON.stringify(entry)).join('\n');
                                    } else {
                                        cgmDataForPayload.entries = ""; // No valid entries after processing
                                        if (settings.ai_llm_debug === true) {
                                            console.log('AI Eval DEBUG: No processed entries to format into JSON Lines. Setting empty string for entries.');
                                        }
                                    }
                                } catch (e) {
                                    if (settings.ai_llm_debug === true) {
                                        console.error('AI Eval DEBUG: Error during entries processing:', e);
                                    }
                                    // cgmDataForPayload.entries remains "Entries data not available..."
                                }
                            } else {
                                if (settings.ai_llm_debug === true) {
                                    console.log('AI Eval DEBUG: dsData.entries is null, undefined, not an array, or empty. Using fallback message for entries.');
                                }
                                // cgmDataForPayload.entries remains "Entries data not available..." if dsData.entries is empty from start
                                if (!(dsData.entries && Array.isArray(dsData.entries))) { // if not an array or doesn't exist
                                    cgmDataForPayload.entries = "Entries data not available or not in expected array format.";
                                } else { // if it is an empty array
                                    cgmDataForPayload.entries = "";
                                }
                            }

                            if (dsData.treatments) {
                                cgmDataForPayload.treatments = dsData.treatments;
                            } else if (settings.ai_llm_debug === true) {
                                console.log('AI Eval DEBUG: No treatments data found in dsData.');
                            }
                            if (dsData.devicestatus) {
                                cgmDataForPayload.deviceStatus = dsData.devicestatus;
                            } else if (settings.ai_llm_debug === true) {
                                console.log('AI Eval DEBUG: No devicestatus data found in dsData.');
                            }

                        } else {
                            if (settings.ai_llm_debug === true) {
                                console.warn('AI Eval DEBUG: reportData.datastorage.data was not found. CGM data parts will use fallback messages.');
                            }
                        }

                        // For the {{CGMDATA}} token, we'll stringify the container object.
                        // The entries part is already a JSON-Lines string.
                        // Treatments and deviceStatus are still objects/arrays, stringified by the outer JSON.stringify.
                        // This is a compromise: entries are compact, other parts remain structured for now.
                        // If treatments also need to be JSON-Lines, cgmDataForPayload.treatments would need similar processing.
                        const cgmDataString = JSON.stringify(cgmDataForPayload, null, 2);
                        ...
                        if (reportData.datastorage) {
                            ...
                            // Priority 2 (Fallback): datastorage.data.profile
                            if (!activeProfile && reportData.datastorage.data && reportData.datastorage.data.profile) {
                                ...
                                if (settings.ai_llm_debug === true) {
                                    console.log('AI Eval: Attempting to use profile from datastorage.data.profile');
                                }

                            }
                            // Priority 3 (Fallback): datastorage.profile (root)
                            if (!activeProfile && reportData.datastorage.profile) {
                                ...
                                if (activeProfile) profileSource = "datastorage.profile (root)";

                                if (settings.ai_llm_debug === true) {
                                    console.log('AI Eval: Attempting to use profile from datastorage.profile (root)');
                                }

                            }
                        }

                        // Priority 4 (Fallback): passedInClient.profile()
                        if (!activeProfile && passedInClient) {
                            const clientProfileFn = passedInClient.profile;

                            if (settings.ai_llm_debug === true) {
                                console.log('AI Eval: Attempting to get profile from passedInClient.profile; type:', typeof clientProfileFn);
                            }

                            if (typeof clientProfileFn === 'function') {
                                const clientProfileResult = clientProfileFn();
                                if (clientProfileResult && typeof clientProfileResult === 'object') {
                                    if (clientProfileResult.store && typeof clientProfileResult.store === 'object') {
                                        activeProfile = clientProfileResult.store;
                                        profileSource = "passedInClient.profile().store";
                                    } else if (Object.keys(clientProfileResult).includes('basal')) {
                                        activeProfile = clientProfileResult;
                                        profileSource = "passedInClient.profile() direct object";
                                    }
                                }

                                if (settings.ai_llm_debug === true) {
                                    console.log('AI Eval: passedInClient.profile() execution result:', clientProfileResult);
                                }

                            } else if (typeof clientProfileFn === 'object' && clientProfileFn !== null) {
                                if (clientProfileFn.store && typeof clientProfileFn.store === 'object') {
                                    activeProfile = clientProfileFn.store;
                                    profileSource = "passedInClient.profile.store (object access)";
                                } else if (Object.keys(clientProfileFn).includes('basal')) {
                                    activeProfile = clientProfileFn;
                                    profileSource = "passedInClient.profile direct object (has basal)";
                                }
                            }
                        }
                        ...
                        if (activeProfile) {
                            ...
                            if (settings.ai_llm_debug === true) {
                                console.log('AI Eval: Extracted profile for {{PROFILE}} from:', profileSource, JSON.stringify(profileToLog, null, 2));
                            }
    ...
    var aiEvalPlugin = {
        ...
        <div id="ai-eval-container" style="padding: 20px;">
            ...
            <p style="font-weight: bold; color: orange;">Warning: Submit a maximum of 2 weeks of data for analysis to ensure optimal performance and accuracy.</p>
            <button id="sendToAiButton" style="margin-top: 5px; padding: 8px 15px;">Send to AI</button>
