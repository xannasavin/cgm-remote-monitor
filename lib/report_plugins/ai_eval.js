                ...
                $.ajax({
                    ...
                    success: function (prompts) {
                        ...
                        const interimJsonContents = window.interimResponses.map(response => {
                            try {
                                // Assuming the JSON is in the first choice's message content
                                if (response && response.choices && response.choices[0] && response.choices[0].message && response.choices[0].message.content) {
                                    return JSON.parse(response.choices[0].message.content);
                                }
                            } catch (e) {
                                console.error('AI Eval: Error parsing interim response content to JSON:', e);
                                return { error: "Failed to parse content", original_content: response.choices[0].message.content };
                            }
                            return null;
                        }).filter(Boolean); // Filter out any nulls from failed parsing

                        const interimDataString = JSON.stringify(interimJsonContents, null, 2);
