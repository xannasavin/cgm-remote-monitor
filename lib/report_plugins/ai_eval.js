    ...
    function initializeAiEvalTab(passedInClient) {
        ...
        if (sendButton) {
            sendButton.addEventListener('click', async function () {
                ...
                if (typeof window === 'undefined' || !window.currentAiEvalInterimPayloads) {
                    console.error('AI Eval: No interim payloads available to send. Was data processed?');
                    alert('AI Evaluation interim payloads are not ready. Please ensure report data is loaded and processed.');
                    return;
                }

                button.textContent = 'Processing Interim... (0/' + window.currentAiEvalInterimPayloads.length + ')';
                button.disabled = true;

                const interimPayloads = window.currentAiEvalInterimPayloads;
                const interimResponses = [];
                const apiEndpoint = (settings.baseURL || '') + '/api/v1/ai_eval';
                const requestHeaders = passedInClient.headers ? passedInClient.headers() : {};
                requestHeaders['Content-Type'] = 'application/json';

                const interimResponseDebugArea = document.getElementById('aiEvalInterimResponseDebugArea');
                if (interimResponseDebugArea) {
                    interimResponseDebugArea.textContent = 'Starting interim AI calls...\n';
                }

                for (let i = 0; i < interimPayloads.length; i++) {
                    const payload = interimPayloads[i];
                    button.textContent = `Processing Interim... (${i + 1}/${interimPayloads.length})`;

                    try {
                        console.log(`AI Eval: Calling interim API for payload ${i + 1}`);
                        if (settings.ai_llm_debug === true) {
                            console.log(`AI Eval: Interim Payload ${i + 1}:`, JSON.stringify(payload, null, 2));
                        }
                        ...
                        if (!response.ok) {
                            ...
                            throw new Error(`Interim call ${i + 1} failed. Status: ${response.status}. Body: ${errorText}`);
                        }

                        const data = await response.json();
                        console.log(`AI Eval: Interim API call ${i + 1} successful. Response data:`, data);
                        interimResponses.push(data);
                        ...
                        console.error('AI Eval: Interim API call failed:', error);
                        if (interimResponseDebugArea) {
                            interimResponseDebugArea.textContent += `\n--- ERROR for Day ${i + 1} ---\n` + error.message;
                        }
                        // Decide if you want to stop on first error or continue
                        // For now, we stop.
                        button.textContent = 'Error during interim calls';
                        button.disabled = false; // Re-enable on error
                        return;
                    }
                }

                console.log('AI Eval: All interim calls completed.');
                button.textContent = 'Send to AI';
                button.disabled = false;

                // Storing responses for the next step (final payload construction)
                if(typeof window !== 'undefined') {
                    window.currentAiEvalInterimResponses = interimResponses;
                }

                // All interim calls are done, now construct and send the final payload
                (async () => {
                    try {
                        button.textContent = 'Constructing Final...';
                        console.log('AI Eval: Constructing final payload.');

                        const systemPrompt = window.currentAiEvalSystemPrompt;
                        let userPromptTemplate = window.currentAiEvalUserPromptTemplate;
                        const cgmProfile = window.currentAiEvalCgmProfile;
                        const interimResponsesContent = interimResponses.map(r => r.html_content || JSON.stringify(r)).join('\n\n---\n\n');

                        let finalUserPrompt = userPromptTemplate.replace('{{CGMDATA}}', interimResponsesContent);
                        finalUserPrompt = finalUserPrompt.replace('{{PROFILE}}', cgmProfile);
                        ...
                        if (settings.ai_llm_debug === true) {
                            const finalDebugArea = document.getElementById('aiEvalDebugArea');
                            if (finalDebugArea) {
                                finalDebugArea.textContent = 'FINAL PAYLOAD (DEBUG):\n\n' + JSON.stringify(finalPayload, null, 2);
                            }
                            console.log('AI Eval: Final Payload:', JSON.stringify(finalPayload, null, 2));
                        }

                        button.textContent = 'Sending Final...';
                        console.log('AI Eval: Sending final payload to API.');
                        ...
                        if (!response.ok) {
                            const errorText = await response.text();
                            throw new Error(`Final API call failed. Status: ${response.status}. Body: ${errorText}`);
                        }

                        const data = await response.json();
                        console.log('AI Eval: Final API call successful. Response data:', data);

                        // Store final response for the next step
                        if(typeof window !== 'undefined') {
                            window.currentAiEvalFinalResponse = data;
                        }

                        const finalResponse = window.currentAiEvalFinalResponse;

                        const outputArea = document.getElementById('aiResponseOutputArea');
                        if (outputArea && finalResponse.html_content) {
                            outputArea.innerHTML = finalResponse.html_content;
                        }

                        const statsArea = document.getElementById('aiStatistics');
                        if (statsArea && finalResponse.tokens_used) {
                            statsArea.textContent = `Tokens Used: ${finalResponse.tokens_used}`;
                        }

                        if (settings.ai_llm_debug === true) {
                            const finalResponseDebugArea = document.getElementById('aiEvalResponseDebugArea');
                            if (finalResponseDebugArea) {
                                finalResponseDebugArea.textContent = 'FINAL RESPONSE (RAW):\n\n' + JSON.stringify(finalResponse, null, 2);
                            }
                        }
                        ...
                        console.error('AI Eval: Final API call failed:', error);
                        const finalResponseDebugArea = document.getElementById('aiEvalResponseDebugArea');
                        if (finalResponseDebugArea) {
                            finalResponseDebugArea.style.display = 'block';
                            finalResponseDebugArea.textContent = 'Final AI API Call Error:\n\n' + error.message;
                        } else {
                            alert('Final AI API Call Error: ' + error.message);
                        }
                        button.textContent = 'Error during final call';
                        button.disabled = false;
                    }
                })();
            });
                    ...
                    if (reportData && reportData.datastorage) {
                        ...
                        if (reportData.datastorage) {
                            ...
                            for (let i = 0; i < cgmData.days.length; i++) {
                                ...
                                let tempUserPromptContent = userInterimPromptContent.replace('{{CGMDATA}}', generateMarkdownFromDay(cgmData.days[i]));
                                tempUserPromptContent = userInterimPromptContent.replace('{{DATE}}', generateMarkdownFromDay(cgmData.days[i].date));
                            ...
                            if (settings.ai_llm_debug === true) {
                                console.log('AI Eval DEBUG: =================================');
                                console.log('AI Eval DEBUG: Interim Payloads per Day: ', interimPayloads);
                                console.log('AI Eval DEBUG: =================================');
                            }

                            debugInterimPayload = JSON.stringify(interimPayloads, null, 2);

                            //Build the String for the prompt to replace {{CGMDATA}} @TODO!!!!!
                            cgmDataString = 'Hier sind die CGM Daten im JSON Format von ';
                            cgmDataString += cgmData.dateFrom;
                            cgmDataString += ' bis ';
                            cgmDataString += cgmData.dateTill;
                            cgmDataString += ' (' + cgmData.numberOfDays + ' Tage).\n';

                            cgmDataString += generateMarkdownFromDays(cgmData.days); //// @JULES: HERE WE NEED TO FIRST HAVE THE INTERIM API CALL RESPONSES SO WE CAN INSERT THEM TO THE FINAL RESPONSE
                            //cgmDataString = JSON.stringify(reportData.datastorage, null, 2); // Fallback

                            if (settings.ai_llm_debug === true) {
                                console.log('AI Eval DEBUG: =================================');
                                console.log('AI Eval DEBUG: cgmDataString: ', cgmDataString);
                                console.log('AI Eval DEBUG: =================================');
                            }
                        ...
                        userPromptContent = userPromptContent.replace('{{PROFILE}}', cgmProfile);

                        if (settings.ai_llm_debug === true) {
                            console.log('AI Eval DEBUG User Prompt after replacements:\n', userPromptContent);
                        }

                        // Store interim payloads and prompts on the window object for the send button
                        if (typeof window !== 'undefined') {
                            window.currentAiEvalInterimPayloads = interimPayloads;
                            window.currentAiEvalSystemPrompt = systemPromptContent;
                            window.currentAiEvalUserPromptTemplate = prompts.user_prompt_template || "Analyze the following CGM data: {{CGMDATA}} using this profile: {{PROFILE}}";
                            window.currentAiEvalCgmProfile = cgmProfile;
                        }

                        if (settings.ai_llm_debug === true) {
                            const debugAreaInterim = document.getElementById('aiEvalInterimDebugArea');
                            if (debugAreaInterim) {
                                debugAreaInterim.textContent = 'AI INTERIM PROMPT PAYLOAD (DEBUG):\n\n' + debugInterimPayload;
                                console.log('AI Eval: Interim Payload displayed in debug area.');
                            }

                            const debugArea = document.getElementById('aiEvalDebugArea');
                            if (debugArea) {
                                debugArea.textContent = 'Final payload will be constructed after interim calls.';
                            }
                        }
                    } else {
