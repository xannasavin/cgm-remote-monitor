...
## Migration notes for custom prompts

If you have customised the prompts in Admin Tools > AI Evaluation Prompt Settings:

- **Remove any `{{RETURNFORMAT}}` block** — the schema comes through `response_format`.
  Leaving it in still works (backwards compat), but costs ~5KB per call.
- **Remove any `{{TREATMENT_SUMMARY}}` block** — the same numbers are already inside
  `{{STATS_JSON}}.period.treatment_summary`. Leaving it in still works, but duplicates.
- **Do not write prompts that assume `per_day[i].stats.time_blocks` exists** — it is
  now stripped from the LLM payload. Hourly context is in `period.diurnal_patterns`.
- **Do not write prompts that assume every treatment row is present** — zero-value
  pump-tick rows are filtered out. Every treatment row in the payload now carries
  at least one of: carbs > 0, insulin > 0, or a non-empty note string.
